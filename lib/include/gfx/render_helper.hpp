#ifndef RENDER_HEALPER_HPP_
#define RENDER_HEALPER_HPP_
...
public:
  Render_Helper(gfx::Renderer& renderer) {
    set_draw_color(renderer, 0, 0, 0, 255);
  }

  ~Render_Helper() {}

  void enqueue_task(const data::Render_Task to_enqueue) {
    tasks_to_render_.push(to_enqueue);
  }
